...
# Usage: ./debug_predicted_steps.sh [directory] [window_size] [window_stride] [input_model.h5] [normalization_type] [gait] [sensor #] [output_histogram.png]
...
if [ "$#" -ne 8 ]; then
    echo "Usage: ./debug_predicted_steps.sh [directory] [window_size] [window_stride] [input_model.h5] [normalization_type] [gait] [sensor #] [output_histogram.png]"
    exit 1
fi
...
python3 generate_histogram.py debug.csv $8
rm debug.csv
