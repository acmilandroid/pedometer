	...
	for d in $3*; do
		if [ -d "$d" ]; then
			...
			for (( sensor=1; sensor<=3; sensor++ )); do
				...
				python3 ../training/test_model.py $2/training75_ALL_ALL_model.h5 $window_size $1/cutnorm_"$window_size"/"$num"_Regular_"$sensor"_norm.txt $d/Regular/steps.txt 0 >> temp_big_test_data/test_results_$window_size.txt
				echo "Testing SemiRegular gait, sensor $sensor..."
				python3 ../training/test_model.py $2/training75_ALL_ALL_model.h5 $window_size $1/cutnorm_"$window_size"/"$num"_SemiRegular_"$sensor"_norm.txt $d/SemiRegular/steps.txt 0 >> temp_big_test_data/test_results_$window_size.txt
				echo "Testing Irregular gait, sensor $sensor..."
				python3 ../training/test_model.py $2/training75_ALL_ALL_model.h5 $window_size $1/cutnorm_"$window_size"/"$num"_Irregular_"$sensor"_norm.txt $d/Irregular/steps.txt 0 >> temp_big_test_data/test_results_$window_size.txt
			done
		fi
	done

	# grab important result data and make temp file
	pcregrep -M "Predicted steps:.*\nActual steps:.*\nDifference in steps:.*\nTP:.*\nFP:.*\nFN:.*\nPPV:.*\nSensitivity:.*\nRCA:.*\nSDA:.*" temp_big_test_data/test_results_$window_size.txt | sed 's/^.*: //' > temp_big_test_data/important_results_$window_size.txt
...
for (( window_size=$WINDOW_START; window_size<=$WINDOW_END; window_size+=$WINDOW_INCREMENT )); do
	...
	for (( subject = 0; subject < $num; subject++ )) do
		...
		for (( sensornum = 0; sensornum < 3; sensornum++ )) do
			...
			echo -n "$print," >> $4
			((line++))
			sed "${line}q;d" temp_big_test_data/important_results_$window_size.txt >> $4
			truncate -s -1 $4
			echo -n "," >> $4
			((line++))
			sed "${line}q;d" temp_big_test_data/important_results_$window_size.txt >> $4
